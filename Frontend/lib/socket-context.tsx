  ...
  const refreshSessions = useCallback(async () => {
    try {
        ...
        normalized.forEach((s: any) => {
            if (s.qr && s.status !== "READY" && s.status !== "ONLINE") {
              restoredQrs[s.id] = s.qr;
            }
        });
        setQrCodes((prev) => {
          const next = { ...prev, ...restoredQrs };
          normalized.forEach((session: Session) => {
            if (session.status === "READY" || session.status === "ONLINE") {
